...
const handleSubmit = async (e) => {
  e.preventDefault();
  try{
    await axios.patch(`http://localhost:5000/kelas/${kode_kelas}`, {
      nama, jumlah
    });
...
const getKelasByKodeKelas = async () => {
    const response = await axios.get(`http://localhost:5000/kelas/${kode_kelas}`);
              ...
              <form onSubmit={handleSubmit}>
                        <div class="card-body">
                            <div class="form-group">
                                <label>Kode Kelas</label><br></br>
                                <input type="number" className="input input-bordered mt-2 input-primary w-full max-w" value={kode_kelas} name="id_kelas" disabled/>
                            </div>
                            <div class="form-group">
                                <label>Kelas</label><br></br>
                                <input type="text" className="input input-bordered mt-2 input-primary w-full max-w" placeholder="Masukkan Kelas"  name="nama_kelas" onChange={e => setnama(e.target.value)} value={nama}/>
                            </div>
                            <div class="form-group">

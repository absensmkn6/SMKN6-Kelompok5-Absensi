...
const handleDelete = (id) => {
  swal({
    ...
    if (willDelete) {
      deleteguru(id).then(() => {
        swal("Sukses! Data Guru telah dihapus", {
          icon: "success",
        });
      }).catch((error) => {
        console.log(error);
            ...
            <div class="rounded-t mb-0 px-0 border-0">
              <div class="flex flex-wrap items-center px-4 py-2">
                <div class="relative w-full max-w-full flex items-center grid grid-cols-6 gap-4">
                  ...
                  <a href="/create/guru" htmlFor="modalTambah" className="btn btn-primary btn-sm mr-2 col-end-7 " >+ Tambah Data</a>
                </div>
                ...
                <table class="items-center w-full bg-transparent border-collapse">
                  <thead>
                    <tr>
                      ...
                      <th class="px-4 bg-gray-100 dark:bg-gray-600 text-gray-500 dark:text-gray-100 align-middle border border-solid border-gray-200 dark:border-gray-500 py-3 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap font-semibold text-left">Jeid Kelamin</th>
...

                  {
                    DataGuru.map((guru, index)=>(
                      <tr key={guru.id} class="text-gray-700 dark:text-gray-100">
                        <td class="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">{index+1}</td>
                        <td class="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">{guru.nama}</td>
                        {/* <td class="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">{data.ttl}</td> */}
                        <td class="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">{guru.kelas}</td>
                        <td class="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">{guru.jurusan}</td>
                        <td class="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">{guru.alamat}</td>
                        <td class="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">{guru.gender}</td>
                        <td class="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">{guru.nohp}</td>
                        ...
                        <td class="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                          <div class="flex items-center">
                            <div class="relative w-full">
                              <Link to={`/detail/guru/${guru.id}`} className="btn btn-outline btn-info btn-sm mr-2" htmlFor="modalDetail">Detail</Link>
                              <Link to={`/edit/guru/${guru.id}`} className="btn btn-outline btn-warning btn-sm mr-2" htmlFor="modalEdit">Edit</Link>
                              <button className="btn btn-outline btn-error btn-sm mr-2" onClick={() => handleDelete(guru.id)}>Hapus</button>
                            </div>
                          </div>
                        </td>
                    </tr>
